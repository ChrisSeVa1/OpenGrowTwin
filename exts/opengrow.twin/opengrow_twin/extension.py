...
from opengrow.display_state import select_display_result  # noqa: E402
from opengrow.manufacturer_profiles import manufacturer_bundle_available  # noqa: E402
...
from .scientific_panel import ScientificPanel  # noqa: E402
...
class OpenGrowTwinExtension(omni.ext.IExt):
    ...
    def on_startup(self, ext_id):
        ...
        self._scientific = ScientificPanel(manufacturer_bundle_available())
    ...
    def _build_window(self):
        self._window = ui.Window("OpenGrowTwin", width=500, height=980)
        with self._window.frame:
            with ui.ScrollingFrame():
                with ui.VStack(spacing=8, height=0):
                    ui.Label("Spectral Lighting Simulation", height=24)
                    self._scientific.build()
                    ui.Separator(height=4)
                    ui.Label("Simulation", height=24)
                    ui.Label("Grid mode")
    ...
    def _selected_optical_model(self):
        return self._scientific.selected_optical_model()

    def _sync_rtx_for_selected_model(self, stage):
        if self._selected_optical_model() == "manufacturer":
            return sync_rtx_lights(stage)
        return sync_rtx_lights(stage, ies_by_channel={})

    def _prepare_design(self, stage, mode):
        return prepare_solver_design(
            stage,
            mode,
            optical_model=self._selected_optical_model(),
        )
    ...
    def _apply_optimizer_clicked(self):
        try:
            ...
            try:
                ...
                self._sync_rtx_for_selected_model(stage)
            finally:
                self._applying_optimizer = False
            self._optimizer_proposal = None
    ...
    def _build_optimizer_proposal(self, objective):
        stage = self._stage()
        design = self._prepare_design(stage, "final")
    ...
    def _displayed_result(self):
        if self._last_result is None:
            return None
        key = tuple(self._last_result["mode_shape"])
        baseline = self._baselines.get(key)
        return select_display_result(self._last_result, baseline, self._display_mode)

    def _refresh_display(self):
        displayed = self._displayed_result()
        if displayed is None:
            return
        metrics = displayed["metrics"]
        blocked = displayed["blocked_ray_count"]
        total = displayed["total_ray_count"]
        self._metrics.text = (
        ...
        self._scientific.update_from_result(displayed)
    ...
    async def _run(self, mode, reason):
        try:
            ...
            self._sync_rtx_for_selected_model(stage)
            design = self._prepare_design(stage, mode)
    ...
    def _show_result(self, result, mode):
        ...
        print(
            ...
            f"optical_model={self._selected_optical_model()}, "
    ...
    def _copilot_inspect_scene(self):
        ...
        return {
            ...
            "active_optical_model": self._selected_optical_model(),
