...
def test_live_scene_contains_discoverable_contract(tmp_path):
    ...
    # The visual fixture housing must sit above the local emitter plane so it
    # cannot physically enclose and shadow the downward-facing RTX DiskLights.
    housing = text.split('def Cube "Housing"', 1)[1].split('def Scope "Emitters"', 1)[0]
    assert 'custom bool opengrow:visualOnly = true' in housing
    assert 'double3 xformOp:translate = (0, 0, 0.03)' in housing
    assert 'uniform token[] xformOpOrder = ["xformOp:translate", "xformOp:scale"]' in housing


def test_scene_scale_contract_is_explicit_and_metric(tmp_path):
    """Lock the physical demo scale because inverse-square physics depends on it."""
    path = tmp_path / "scene.usda"
    write_live_scene_usda(_design(), path)
    text = path.read_text(encoding="utf-8")

    assert 'metersPerUnit = 1' in text
    assert 'upAxis = "Z"' in text

    fixture = text.split('def Xform "Fixture_01"', 1)[1].split('def Scope "SensorPlanes"', 1)[0]
    assert 'double3 xformOp:translate = (0, 0, 0.6)' in fixture

    housing = text.split('def Cube "Housing"', 1)[1].split('def Scope "Emitters"', 1)[0]
    assert 'double3 xformOp:scale = (0.96, 0.56, 0.05)' in housing

    canopy = text.split('def Cube "CanopyPlane"', 1)[1].split('def Scope "Occluders"', 1)[0]
    assert 'custom double opengrow:widthM = 1' in canopy
    assert 'custom double opengrow:depthM = 0.6' in canopy
    assert 'double3 xformOp:scale = (1, 0.6, 0.01)' in canopy
