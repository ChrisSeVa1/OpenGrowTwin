...
def test_parse_and_normalize_tabulated_spectrum():
    ...
    area = np.trapezoid(density, spectrum.wavelengths_nm)
...
def test_radiant_spectral_density_preserves_total_flux():
    ...
    assert np.trapezoid(spectral_flux, spectrum.wavelengths_nm) == pytest.approx(1.448)
...
def test_band_integration_interpolates_non_sample_boundaries():
    spectrum = parse_spectrum_text("400 0\n500 1\n600 0\n")
    # The interval 450–550 nm cuts through two source segments. Interpolating
    # both boundaries gives a symmetric trapezoid with half the total area.
    assert spectrum.fraction_in_band(450.0, 550.0) == pytest.approx(0.75)


def test_photon_band_outside_source_support_is_zero():
    spectrum = parse_spectrum_text("400 1\n450 1\n500 1\n")
    assert spectrum.photon_flux_per_watt_umol_s(700.0, 750.0) == pytest.approx(0.0)
