...
def test_ies_mapping_auto_resolves_local_bundle_when_available():
    mapping = _normalized_ies_mapping(None)
    assert set(mapping) == {"blue", "red", "far_red"}
    assert mapping["blue"].endswith("GD_PUBRA1_15_20250529.ies")
    assert mapping["red"].endswith("GH_PUBRA1_25_20250526.ies")
    assert mapping["far_red"].endswith("GF_PUBRA1_25_20250603.ies")


def test_ies_mapping_preserves_explicit_asset_paths():
