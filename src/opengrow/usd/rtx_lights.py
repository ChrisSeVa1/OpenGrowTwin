...
def _normalized_ies_mapping(ies_by_channel: dict | None) -> dict[str, str]:
    ...
    if ies_by_channel is None:
        from opengrow.manufacturer_profiles import manufacturer_ies_by_channel

        ies_by_channel = manufacturer_ies_by_channel(required=False)
...
def sync_rtx_lights(
    ...
    asset paths. If omitted, OpenGrowTwin attempts to resolve its complete local,
    gitignored manufacturer bundle. When an explicit empty mapping is supplied,
    existing IES shaping is removed so simplified scientific/RTX A/B mode is real.

    The same scientific emitter remains authoritative for transform, power,
    authored enabled state, and effective USD visibility. RTX lights remain
    ...
    for prim in list(stage.Traverse()):
        ...
        authored_enabled = bool(prim.GetAttribute("opengrow:enabled").Get())
        visible = UsdGeom.Imageable(prim).ComputeVisibility() != UsdGeom.Tokens.invisible
        enabled = authored_enabled and visible
        ...
        if ies_file is not None:
            ...
            if light_prim.HasAPI(UsdLux.ShapingAPI):
                light_prim.RemoveAPI(UsdLux.ShapingAPI)
            if light_prim.HasProperty("opengrow:iesAssetPath"):
                light_prim.RemoveProperty("opengrow:iesAssetPath")
            light_prim.CreateAttribute("opengrow:angularModel", Sdf.ValueTypeNames.Token, custom=True).Set(
